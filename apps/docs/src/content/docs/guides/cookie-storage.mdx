---
title: Cookie & Storage
description: Configure how consent is persisted — cookies, localStorage, and custom domains.
---

## Default behavior

By default, the library stores consent in a cookie named `cc_cookie` with these settings:

```ts
{
  name: "cc_cookie",
  expiresAfterDays: 182,    // ~6 months
  domain: "",               // Auto: location.hostname
  path: "/",
  secure: true,
  sameSite: "Lax",
}
```

## Customizing the cookie

Override any of these in your config:

```ts
const consent = createConsent({
  cookie: {
    name: "my-consent-cookie",
    expiresAfterDays: 365,
    domain: ".example.com", // Share across subdomains
    path: "/",
    secure: true,
    sameSite: "Strict",
  },
  categories: {
    necessary: { readOnly: true },
    analytics: {},
  } as const,
});
```

### Dynamic expiration

`expiresAfterDays` can be a function. For example, set longer expiration when the user accepts
all categories:

```ts
cookie: {
  expiresAfterDays: (acceptType) => {
    if (acceptType === "all") return 365;
    if (acceptType === "necessary") return 90;
    return 182;
  },
},
```

## Using localStorage

Switch from cookies to `localStorage` by providing the `localStorageStorage()` adapter:

```ts
import { createConsent, localStorageStorage } from "@uabcodus/consent";

const consent = createConsent({
  storage: localStorageStorage("cc_cookie"),
  categories: {
    necessary: { readOnly: true },
    analytics: {},
  } as const,
});
```

The first argument is the localStorage key name. Optionally pass an expiry in milliseconds as the second argument (defaults to 182 days).

### Using cookie storage explicitly

The default storage is cookie-based, but you can also configure it explicitly:

```ts
import { createConsent, cookieStorage } from "@uabcodus/consent";

const consent = createConsent({
  storage: cookieStorage({
    name: "my-consent-cookie",
    expiresAfterDays: 365,
    domain: ".example.com",
    path: "/",
    secure: true,
    sameSite: "Strict",
  }),
  categories: {
    necessary: { readOnly: true },
    analytics: {},
  } as const,
});
```

When no `storage` is specified, `cookieStorage()` is used with the options from the `cookie` config field.
This means you can still use the shorthand `cookie` key for cookie-specific settings:

```ts
const consent = createConsent({
  cookie: { name: "my-cookie", expiresAfterDays: 90 },
  categories: {
    necessary: { readOnly: true },
    analytics: {},
  } as const,
});
```

### Custom storage adapters

The `StorageAdapter` interface lets you plug in any persistence mechanism:

```ts
interface StorageAdapter {
  get(): CookieValue;
  set(value: CookieValue): void;
  remove(): void;
}
```

For example, you could implement sessionStorage, IndexedDB, or a server-side backend.

When using localStorage, the library also stores an `expirationTime` and checks it on page load.
Expired consent is treated as invalid, same as with cookies.

## Reading cookie data

Use `getCookie()` or `getCookie(field)` to read consent data:

```ts
const cookie = consent.getCookie();
// { categories: [...], services: {...}, consentId: "...", ... }

const consentId = consent.getCookie("consentId");
// "a1b2c3d4-..."

const timestamp = consent.getCookie("consentTimestamp");
// "2025-01-01T00:00:00.000Z"
```

## Storing custom data

Need to store additional data alongside consent? Use `setCookieData()`:

```ts
// Replace the entire data field
consent.setCookieData({ value: { userId: "123" }, mode: "set" });

// Merge into existing data (shallow merge for objects)
consent.setCookieData({ value: { theme: "dark" }, mode: "update" });
```

The data is accessible later:

```ts
const data = consent.getCookie("data");
// { userId: "123", theme: "dark" }
```

Returns `true` if anything actually changed (so you can avoid unnecessary writes).

## Erasing cookies

Manually erase cookies by name, RegExp, or array of both:

```ts
// Single cookie
consent.eraseCookies("_ga");

// Regex — erase all GA cookies
consent.eraseCookies(/^_ga/);

// Array
consent.eraseCookies(["_ga", "_gid", /^_gat/]);

// Custom path and domain
consent.eraseCookies("_ga", "/subdir", ".example.com");
```

This is handled automatically for rejected categories when `autoClearCookies` is enabled (default:
`true`). But you can call it manually too — useful for cleaning up cookies from a previous consent
plugin.

## Subdomain cookies

To share consent across subdomains, set the `domain` to the root domain with a leading dot:

```ts
cookie: {
  domain: ".example.com",
}
```

This makes the cookie available on `www.example.com`, `app.example.com`, `blog.example.com`, etc.

For `localhost` development, leave `domain` empty (default) — most browsers ignore domain on
`localhost`.

## Next steps

- [Revision Management →](/guides/revision-management/) — force re-consent on policy changes
- [API Reference →](/reference/core/) — full list of cookie-related methods
