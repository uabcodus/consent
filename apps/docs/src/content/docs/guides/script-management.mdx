...
Add `data-category` to any `<script>` tag. Change the `type` to `"text/plain"` so the browser
...
`type="text/plain"` and `data-category` attributes) so the browser executes it.
...
```html
<script type="text/plain" data-category="marketing" data-service="facebookPixel">
