...
export async function fetchLatestRelease() {
  ...
  const res = await fetch(
    "https://raw.githubusercontent.com/orestbida/cookieconsent/master/package.json",
  );
...
export async function updateReleaseSpans() {
  ...
  for (const el of releaseSpans) {
    el.textContent = el.hasAttribute("data-minor-release") ? getMinor(latest) : latest;
  }
}
...
function validRelease(release) {
   ...
   */
  const safeRelease = !["alpha", "beta", "test"].some((str) => release.includes(str));
