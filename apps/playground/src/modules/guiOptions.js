...
function updateGuiOptionsState(modalName, key, value) {
  ...
  if (modalName === CONSENT_MODAL_NAME) {
    if (key === "layout") {
      ...
      if (
        (!isBarLayout && validBarPositions(currentPosition)) ||
        (isBarLayout && !validBarPositions(currentPosition))
      ) {
        ...
        if (isBarLayout) {
          if (!validBarPositions(currentPosition))
            consentModal.position = htmlElements[CONSENT_MODAL_NAME].position.value =
              state._lastBarPosition || "bottom";
        } else {
          consentModal.position = htmlElements[CONSENT_MODAL_NAME].position.value =
            state._lastNonBarPosition || defaultConfig.guiOptions.consentModal.position;
        }
      }
    }
    ...
    if (key === "layout") {
      if (currentPMLayoutValue === "box") disablePMPositionOptions(true);
      else if (pmPositionOptionsDisabled) disablePMPositionOptions(false);
    }
  }
...
function setPosition(modalName, position, layout) {
  ...
  if (modalName === CONSENT_MODAL_NAME) generateCMPositionOptions(!layout.startsWith("bar"));
...
function generateCMPositionOptions(all) {
  ...
  if (all) {
    newOptions = generateOptions(cmLayoutYValues, cmLayoutXValues);
  } else {
