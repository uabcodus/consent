...
/** @type {HTMLSpanElement} **/ const currentActiveLanguageSpan =
  getById("current-active-language");
...
onEvent(customEvents._PLAYGROUND_READY, () => {
  ...
  const currLang =
    autoDetect && enabledTranslation(detectedLanguage, state) ? detectedLanguage : language.default;
...
export function updateDefaultLanguage(lang) {
  if (!lang) defaultLanguageSelect.value = "disabled";

  if (allLanguages.includes(lang)) defaultLanguageSelect.value = lang;
}
...
export function updateDefaultLanguageOptions(languages) {
  ...
  for (const option of options) {
    option.style.display = languages.includes(option.value) ? "" : "none";
  }
}
...
export function updateTranslationFound(found) {
  translationFoundSpan.textContent = found ? "true" : "false";
}
...
export function detectLanguage(mode) {
  if (mode === "browser") return getBrowserLanguage();

  if (mode === "document") return getDocumentLanguage();
...
export function updateCurrentLanguage(languageCode, newState) {
  ...
  const currLanguage = enabledTranslation(languageCode, state) ? languageCode : "-";
