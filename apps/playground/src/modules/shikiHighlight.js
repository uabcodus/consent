...
function shikiHighlightBlock(html, shiki, props) {
  ...
  let bodyCleaned = body
    .map((row) => {
      ...
      return (
        !emptyRow && row.slice(0, indentationSpaces).trim() === ""
  ...
  if (showLineNumbers) {
    ...
    for (let i = 1; i <= nRows; i++) {
      ...
      if (shouldFocusLines && focusIndexes[i] === true) className.push("line--focus");

      if (shouldHighlightLines && highlightIndexes[i] === true) className.push("line--highlight");
    ...
    transformers: [
      {
        line: (Element, index) => {
          ...
          if (shouldFocusLines && focusIndexes[index]) className += " line--focus";
  ...
  const tag = title && isDetails ? "details" : "div";

  const titleTag = title && isDetails ? "summary" : "div";
  ...
  const renderedCodeBlock = `
    <${tag}
        class="${className}"
    >
        ${title ? `<${titleTag} class="hl__title">${title.trim()}</${titleTag}>` : ""}
...
function createCustomBlock(html) {
  ...
  for (const block of codeBlocks) {
    ...
    const className =
      `custom-block custom-block--${type}` + (isDetails ? " custom-block--details" : "");

    const customBlock = `
        <${tag}
            class="${className}"
        >
            ${title ? `<${titleTag} class="custom-block__title">${title.trim()}</${titleTag}>` : ""}
...
function getIndexes(rangeStr) {
   ...
   * @type {string[]}
   */
  const split = rangeStr?.trim().replaceAll(" ", "").split(",") || [];
