...
export const resetCookies = () => {
  ...
  if (cc.validConsent()) cc.acceptCategory([]);
...
export const saveState = (newState) => {
  try {
    newState && localStorage.setItem(DEMO_ITEM_NAME, JSON.stringify(newState));
  } catch (e) {
  ...
  if (savedState) {
    try {
      ...
      for (let key in defaultState) {
        if (typeof savedState[key] !== typeof defaultState[key]) return resetState();
      }

      if (savedState._demoRevision !== defaultState._demoRevision) return resetState();
    } catch (e) {
      return resetState();
    }
  }
...
export const getCurrentUserConfig = (state) => {
  ...
  if (!config.disablePageInteraction) delete config.disablePageInteraction;
  ...
  for (const languageCode in allTranslations) {
    if (!state._enabledTranslations.includes(languageCode)) {
      ...
      const filteredSections = preferencesModal.sections.filter((section) => {
        ...
        return !section.linkedCategory || state._enabledCategories.includes(section.linkedCategory);
      });
  ...
  if (!("ar" in allTranslations)) delete config.language.rtl;
