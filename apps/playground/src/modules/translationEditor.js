...
function updateEditor(enabledTranslations) {
   ...
   */
  if (enabledTranslations.length === 0) return false;

  /**
   * If there are no changes, don't recreate buttons
   */
  if (equalStrArrays(lastEnabledTranslations, enabledTranslations)) return;
...
function updateEditorFields(_state) {
  ...
  for (const section of sections) {
    const shouldCreateSection =
      !section.linkedCategory || state._enabledCategories.includes(section.linkedCategory);
