  ...
  translationInputs.forEach((input) => {
    addEvent(input, "change", () => {
      ...
      if (enabled) {
        ...
        languageFound &&
          (state._enabledTranslations = state._enabledTranslations.filter(
...
function toggleMissingTranslationError(enabledTranslations) {
  ...
  translationsSection.classList[error ? "add" : "remove"]("no-translations-error");

  editTranslationsBtn.disabled = error;
  editTranslationsBtn.textContent = error ? "Translation required" : editTranslationsBtnText;
}
