...
export const showPreferences = () => {
  ...
  if (!state._preferencesModalExists) createPreferencesModal(miniAPI, createMainContainer);
...
const discardUnsavedPreferences = () => {
  ...
  for (const category in categoryInputs) {
    ...
    categoryInputs[category].checked =
      isReadOnly ||
      (consentIsValid ? acceptedCategory(category) : categoryEnabledByDefault(category));

    for (const service in serviceInputs[category]) {
      serviceInputs[category][service].checked =
        isReadOnly ||
        (consentIsValid ? acceptedService(service, category) : categoryEnabledByDefault(category));
    }
  }
};
...
export const setLanguage = async (newLanguageCode, forceUpdate) => {
  ...
  if (newLanguageCode !== getCurrentLanguageCode() || forceUpdate === true) {
    ...
    if (state._consentModalExists) createConsentModal(miniAPI, createMainContainer);
...
export const run = async (userConfig) => {
  ...
  if (!win._ccRun) {
    ...
    if (_config.mode === OPT_OUT_MODE) manageExistingScripts(_state._defaultEnabledCategories);
  }
};
...
export const reset = (deleteCookie) => {
  ...
  if (deleteCookie) {
    useLocalStorage ? localStorageManager._removeItem(name) : eraseCookies(name, path, domain);
  }
