...
export const setConfig = (userConfig) => {
  ...
  _dom._rootEl =
    (_root && isString(_root) ? doc.querySelector(_root) : _root) || doc.documentElement;
  //{{END: GUI}}
  ...
  if (typeof autoClearCookies === "boolean") config.autoClearCookies = autoClearCookies;

  if (typeof manageScriptTags === "boolean") config.manageScriptTags = manageScriptTags;
  ...
  if (typeof lazyHtmlGeneration === "boolean") config.lazyHtmlGeneration = lazyHtmlGeneration;
  ...
  if (config.hideFromBots === true && nav)
    state._botAgentDetected =
      (nav.userAgent && /bot|crawl|spider|slurp|teoma/i.test(nav.userAgent)) || nav.webdriver;

  if (isObject(userCookieConfig)) config.cookie = { ...cookie, ...userCookieConfig };
