...
export const autoclearCookiesHelper = (isFirstConsent) => {
  ...
  for (const currentCategoryName of categoriesToClear) {
    ...
    const shouldClearCookies = isFirstConsent ? categoryIsDisabled : categoryWasJustDisabled;

    if (!shouldClearCookies) continue;

    if (autoClear.reloadPage && categoryWasJustDisabled) state._reloadPage = true;
...
export const saveCookiePreferences = () => {
  ...
  if (state._invalidConsent || stateChanged) {
    ...
    if (isAutoClearEnabled && shouldClearCookies) autoclearCookiesHelper(isFirstConsent);
...
export const setCookie = (useRemainingExpirationTime) => {
  ...
  useLocalStorage ? localStorageManager._setItem(name, value) : (document.cookie = cookieStr);
...
export const parseCookie = (value, skipDecode) => {
  ...
  parsedValue =
    safeRun(() => JSON.parse(skipDecode ? value : decodeURIComponent(value)), true) || {};
...
export const eraseCookiesHelper = (cookies, customPath, customDomain) => {
  ...
  const erase = (cookie, domain) => {
    if (domain && domain.slice(0, 1) !== ".") domain = "." + domain;
    document.cookie =
      cookie +
      "=" +
      "; path=" +
