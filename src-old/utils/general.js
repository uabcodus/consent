...
export const retrieveRejectedServices = () => {
  ...
  const { _allCategoryNames, _allDefinedServices, _acceptedServices } = globalObj._state;
...
export const resolveEnabledCategories = (categories, excludedCategories) => {
  ...
  if (!categories) {
    ...
    if (isArray(categories)) {
      ...
      enabledCategories = categories === "all" ? _allCategoryNames : [categories];
    }
  ...
  enabledCategories = enabledCategories.filter(
    (category) =>
      !elContains(_allCategoryNames, category) || !elContains(excludedCategories, category),
  );
...
export const resolveEnabledServices = (relativeCategory) => {
  ...
  const categoriesToConsider = relativeCategory ? [relativeCategory] : _allCategoryNames;
  ...
  for (const categoryName of categoriesToConsider) {
    ...
    const customServicesSelection =
      _enabledServices[categoryName] && _enabledServices[categoryName].length > 0;
...
export const updateModalToggles = (service, category) => {
  const state = globalObj._state;
  const { _allDefinedServices, _enabledServices, _preferencesModalExists } = state;
  ...
  if (isString(service)) {
    if (service === "all") {
      ...
      if (elContains(allServiceNames, service)) _enabledServices[category].push(service);
...
export const getRemainingExpirationTimeMS = () => {
  ...
  const elapsedTimeMilliseconds = lastTimestamp ? new Date() - lastTimestamp : 0;
...
export const arrayDiff = (arr1, arr2) => {
  ...
  return a.filter((x) => !elContains(b, x)).concat(b.filter((x) => !elContains(a, x)));
};
...
export const resolveAcceptType = () => {
  ...
  else if (nAcceptedCategories === _readOnlyCategories.length) type = "necessary";
...
export const getCurrentCategoriesState = () => {
  const { _invalidConsent, _acceptedCategories, _allCategoryNames } = globalObj._state;

  return {
    accepted: _acceptedCategories,
    rejected: _invalidConsent
      ? []
      : _allCategoryNames.filter((category) => !elContains(_acceptedCategories, category)),
  };
};
...
export const fireEvent = (eventName, modalName, modal) => {
  const { _onChange, _onConsent, _onFirstConsent, _onModalHide, _onModalReady, _onModalShow } =
