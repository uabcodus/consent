...
export const manageExistingScripts = (defaultEnabledCategories) => {
  ...
  for (const categoryName of _allCategoryNames) {
    const lastChangedServices =
      _lastChangedServices[categoryName] || _acceptedServices[categoryName] || [];
    ...
    for (const serviceName of lastChangedServices) {
      ...
      if (!service._enabled && elContains(_acceptedServices[categoryName], serviceName)) {
        service._enabled = true;
        isFunction(onAccept) && onAccept();
      } else if (service._enabled && !elContains(_acceptedServices[categoryName], serviceName)) {
  ...
  const acceptedCategories = defaultEnabledCategories || _savedCookieContent.categories || [];
